...
impl RegisterFile {
    ...
    pub fn load_from_bytes(data: &[u8], pos: &mut usize) -> crate::gbc::error::Result<Self> {
        ...
        let d = data[*pos]; *pos += 1;
        let e = data[*pos]; *pos += 1;
        let h = data[*pos]; *pos += 1;
        let l = data[*pos]; *pos += 1;
        let pc = u16::from_le_bytes([data[*pos], data[*pos + 1]]); *pos += 2;
        let sp = u16::from_le_bytes([data[*pos], data[*pos + 1]]); *pos += 2;
        Ok(Self { A: a, F: f, B: b, C: c, D: d, E: e, H: h, L: l, PC: pc, SP: sp })
    }
}
