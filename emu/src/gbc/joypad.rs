...
impl Joypad {
    ...
    pub fn save_to_bytes(&self, buf: &mut Vec<u8>) {
        ...
        buf.push(self.directions);
        buf.push(self.selection as u8);
    }

    pub fn load_from_bytes(data: &[u8], pos: &mut usize) -> crate::gbc::error::Result<Self> {
        let buttons = data[*pos]; *pos += 1;
        let directions = data[*pos]; *pos += 1;
        let selection_val = data[*pos]; *pos += 1;
        let selection = match selection_val {
            0 => JoypadSelection::None,
            1 => JoypadSelection::Buttons,
            2 => JoypadSelection::Directions,
            _ => return Err(crate::gbc::error::Error::InvalidValue(format!("Invalid joypad selection: {}", selection_val))),
        };
        Ok(Self { buttons, directions, selection })
    }
}
