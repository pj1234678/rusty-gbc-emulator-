...
use crate::gbc::error::{CartridgeError, Error, Result};
use crate::gbc::memory::{MemoryRead, MemoryWrite};
use crate::gbc::rtc::Rtc;
...
impl Ram {
    ...
    pub(crate) fn set_bank(&mut self, bank: u8) {
        if self.num_banks == 1 {
            eprintln!("Warning: Switching bank on unbanked RAM!");
        }
    ...
    pub fn save_to_bytes(&self, buf: &mut Vec<u8>) {
        ...
        buf.push(self.ram_size as u8);
        buf.push(self.is_dirty as u8);
    }
    ...
    pub fn load_from_bytes(data: &[u8], pos: &mut usize) -> Result<Self> {
        ...
        let data = data[*pos..*pos + data_len as usize].to_vec(); *pos += data_len as usize;
        let active_bank = data[*pos]; *pos += 1;
        let num_banks = data[*pos]; *pos += 1;
        let ram_size_val = data[*pos]; *pos += 1;
        let is_dirty = data[*pos] != 0; *pos += 1;
        let ram_size = match ram_size_val {
            0 => RamSize::NotPresent,
            1 => RamSize::_2K,
            2 => RamSize::_8K,
            3 => RamSize::_32K,
            4 => RamSize::_128K,
            5 => RamSize::_64K,
            _ => return Err(Error::InvalidValue(format!("Invalid RAM size: {}", ram_size_val))),
        };
        Ok(Self { data, active_bank, num_banks, ram_size, is_dirty })
    }
}
...
impl Rom {
    ...
    pub fn update_bank(&mut self, bank: u16) {
        self.active_bank_1 = bank % self.num_banks;
    }

    pub fn save_to_bytes(&self, buf: &mut Vec<u8>) {
        buf.extend_from_slice(&self.active_bank_0.to_le_bytes());
        buf.extend_from_slice(&self.active_bank_1.to_le_bytes());
        buf.extend_from_slice(&self.num_banks.to_le_bytes());
        buf.push(self.rom_size as u8);
    }

    pub fn load_from_bytes(data: &[u8], pos: &mut usize) -> Result<Self> {
        let active_bank_0 = u16::from_le_bytes([data[*pos], data[*pos + 1]]); *pos += 2;
        let active_bank_1 = u16::from_le_bytes([data[*pos], data[*pos + 1]]); *pos += 2;
        let num_banks = u16::from_le_bytes([data[*pos], data[*pos + 1]]); *pos += 2;
        let rom_size_val = data[*pos]; *pos += 1;
        let rom_size = match rom_size_val {
            ...
            0x53 => RomSize::_1_2M,
            0x54 => RomSize::_1_5M,
            _ => return Err(Error::InvalidValue(format!("Invalid ROM size: {}", rom_size_val))),
        };
        let size = usize::from(rom_size);
        Ok(Self { data: vec![0; size], active_bank_0, active_bank_1, num_banks, rom_size })
    }
}
...
pub struct Controller {
    ...
    /// MBC7 EEPROM state
    mbc7_eeprom_reg: u8,     // Current 0xA080 register value
    mbc7_cs: bool,           // Chip select state
    mbc7_clk: bool,          // Clock state
    mbc7_shift: u16,         // Internal shift register
    mbc7_bits_shifted: u8,   // Number of bits shifted in
    mbc7_do: bool,           // Data out bit
    mbc7_eeprom: Vec<u16>,   // 128 x 16-bit EEPROM words
    mbc7_read_data: u16,     // Data being shifted out for READ
    mbc7_read_bits_left: u8, // Bits remaining to shift out
}
...
impl Controller {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            mbc7_bits_shifted: 0,
            mbc7_do: false,
            mbc7_eeprom: vec![0u16; 128],
            mbc7_read_data: 0,
            mbc7_read_bits_left: 0,
        }
    }
    ...
    pub fn from_cartridge(cartridge: Cartridge) -> Result<Self> {
        ...
        let ram_size = if cartridge_type.is_mbc7() {
            // MBC7 always has 8KB built-in RAM regardless of header
            RamSize::_8K
        } else {
            cartridge.ram_size()?
        };
    ...
    pub fn save_to_bytes(&self, buf: &mut Vec<u8>) {
        buf.push(self.boot_rom.is_some() as u8);
        self.rom.save_to_bytes(buf);
        match &self.ram {
            None => buf.push(0),
            Some(ram) => {
                buf.push(1);
                ram.save_to_bytes(buf);
            }
        }
        buf.push(self.rom_size as u8);
        buf.push(self.ram_size as u8);
        buf.push(self.cartridge_type as u8);
        match &self.rtc {
            None => buf.push(0),
            Some(rtc) => {
                buf.push(1);
                let rtc_data = rtc.dump();
                buf.extend_from_slice(&(rtc_data.len() as u32).to_le_bytes());
                buf.extend_from_slice(&rtc_data);
            }
        }
        buf.push(self.rtc_active as u8);
        buf.push(self.banking_mode as u8);
        buf.push(self.ram_enable as u8);
        buf.push(self.ram_rom_bank);
    }

    pub fn load_from_bytes(data: &[u8], pos: &mut usize) -> Result<Self> {
        let _boot_rom_present = data[*pos] != 0; *pos += 1;
        let rom = Rom::load_from_bytes(data, pos)?;
        let ram = if data[*pos] != 0 {
            *pos += 1;
            Some(Ram::load_from_bytes(data, pos)?)
        } else {
            *pos += 1;
            None
        };
        let rom_size_val = data[*pos]; *pos += 1;
        let ram_size_val = data[*pos]; *pos += 1;
        let cartridge_type_val = data[*pos]; *pos += 1;
        let rtc = if data[*pos] != 0 {
            *pos += 1;
            let rtc_len = u32::from_le_bytes([data[*pos], data[*pos+1], data[*pos+2], data[*pos+3]]); *pos += 4;
            let rtc_data = &data[*pos..*pos + rtc_len as usize]; *pos += rtc_len as usize;
            Some(Rtc::from_bytes(rtc_data)?)
        } else {
            *pos += 1;
            None
        };
        let rtc_active = data[*pos] != 0; *pos += 1;
        let banking_mode = data[*pos] != 0; *pos += 1;
        let ram_enable = data[*pos] != 0; *pos += 1;
        let ram_rom_bank = data[*pos]; *pos += 1;
        ...
        let cartridge_type = match cartridge_type_val {
            0 => CartridgeType::Rom,
            1 => CartridgeType::Mbc1,
            2 => CartridgeType::Mbc1Ram,
            3 => CartridgeType::Mbc1RamBattery,
            5 => CartridgeType::Mbc2,
            6 => CartridgeType::Mbc2Battery,
            8 => CartridgeType::RomRam,
            9 => CartridgeType::RomRamBattery,
            0xB => CartridgeType::Mmm01,
            0xC => CartridgeType::Mmm01Ram,
            0xD => CartridgeType::Mmm01RamBattery,
            0xF => CartridgeType::Mbc3TimerBattery,
            0x10 => CartridgeType::Mbc3TimerRamBattery,
            0x11 => CartridgeType::Mbc3,
            0x12 => CartridgeType::Mbc3Ram,
            0x13 => CartridgeType::Mbc3RamBattery,
            0x15 => CartridgeType::Mbc4,
            0x16 => CartridgeType::Mbc4Ram,
            0x17 => CartridgeType::Mbc4RamBattery,
            ...
            _ => return Err(Error::InvalidValue(format!("Invalid cartridge type: {}", cartridge_type_val))),
        };
    ...
    fn read(&self, addr: u16) -> u8 {
        match addr {
            ...
            Ram::BASE_ADDR..=Ram::LAST_ADDR => {
                if self.cartridge_type.is_mbc7() {
                    // MBC7: 0xA080 is the EEPROM control register
                    // Addresses where (addr & 0x0F0F) == 0x0080 map to this register
                    if addr & 0x0F0F == 0x0080 {
                        // Return register with DO (bit 0) set from EEPROM state
                        (self.mbc7_eeprom_reg & 0xFE) | (self.mbc7_do as u8)
                    } else if addr & 0xF000 == 0xA000 {
                        // MBC7 SRAM: 0xA000-0xA0FF (lower 256 bytes)
                        // For now, return 0xFF for unmapped MBC7 addresses
                        0xFF
                    } else {
                        // 0xB000-0xBFFF: regular SRAM banked access
                        match &self.ram {
                            Some(ram) => ram.read(addr),
                            None => 0xFF,
                        }
                    }
                } else if !self.rtc_active {
                    match &self.ram {
                        Some(ram) => ram.read(addr),
                        None => 0xFF,
                    }
...
impl MemoryWrite<u16, u8> for Controller {
    ...
    fn write(&mut self, addr: u16, value: u8) {
        match addr {
            ...
            0x0000..=0x1FFF if self.cartridge_type.is_mbc5() || self.cartridge_type.is_mbc7() => {
                // Cartridge RAM enable/disable
                self.ram_enable = value == 0b1010;
            }
            0x2000..=0x2FFF if self.cartridge_type.is_mbc5() || self.cartridge_type.is_mbc7() => {
                // MBC5 ROM bank select (lower 8 bits)
                let value = (self.rom.active_bank_1 & !0xFF) | value as u16;
                self.rom.update_bank(value);
            }
            0x3000..=0x3FFF if self.cartridge_type.is_mbc5() || self.cartridge_type.is_mbc7() => {
                // MBC5 ROM bank select (9th bit)
                let value = (self.rom.active_bank_1 & !0x100) | ((value as u16 & 0x1) << 8);
                self.rom.update_bank(value);
            }
            0x4000..=0x5FFF if self.cartridge_type.is_mbc5() || self.cartridge_type.is_mbc7() => {
            ...
            Ram::BASE_ADDR..=Ram::LAST_ADDR if !self.rtc_active => {
                if self.cartridge_type.is_mbc7() && addr & 0x0F0F == 0x0080 {
                    // MBC7 EEPROM control register at 0xA080 (and mirrors)
                    let old_cs = self.mbc7_cs;
                    let old_clk = self.mbc7_clk;
                    let new_cs = (value >> 7) & 1 == 1;
                    let new_clk = (value >> 6) & 1 == 1;
                    let new_di = (value >> 1) & 1 == 1;

                    // Clock rising edge with CS high: shift in DI bit
                    if new_cs && !old_clk && new_clk {
                        self.mbc7_shift = (self.mbc7_shift << 1) | (new_di as u16);
                        self.mbc7_bits_shifted = self.mbc7_bits_shifted.saturating_add(1);
                    }

                    // CS low→high: latch and process command
                    if !old_cs && new_cs {
                        // Process the command in the shift register
                        if self.mbc7_bits_shifted >= 10 {
                            let cmd = (self.mbc7_shift >> (self.mbc7_bits_shifted - 10)) & 0x3FF;
                            let start_bit = (cmd >> 9) & 1;
                            let opcode = (cmd >> 7) & 0x3;
                            let addr7 = (cmd >> 1) & 0x7F;
                            let _bit0 = cmd & 1;

                            if start_bit == 1 {
                                match opcode {
                                    0b10 => {
                                        // READ: shift out 16 bits from EEPROM[addr]
                                        let addr = (addr7 as usize) % self.mbc7_eeprom.len();
                                        self.mbc7_read_data = self.mbc7_eeprom[addr];
                                        self.mbc7_read_bits_left = 16;
                                        self.mbc7_do = (self.mbc7_read_data >> 15) & 1 == 1;
                                        self.mbc7_read_data <<= 1;
                                        self.mbc7_read_bits_left -= 1;
                                    }
                                    0b01 => {
                                        // WRITE: EEPROM busy, then ready
                                        // For simplicity, mark as ready immediately
                                        self.mbc7_do = true;
                                    }
                                    _ => {
                                        // EWEN/EWDS/ERASE/ERAL/WRAL - just mark ready
                                        self.mbc7_do = true;
                                    }
                                }
                            } else {
                                // No start bit - treat as idle/ready
                                self.mbc7_do = true;
                            }
                        } else {
                            // Not enough bits shifted - treat as idle/ready
                            self.mbc7_do = true;
                        }
                        self.mbc7_bits_shifted = 0;
                        self.mbc7_shift = 0;
                    }

                    // CS high→low: end of transaction
                    if old_cs && !new_cs {
                        // Nothing special needed
                    }

                    self.mbc7_cs = new_cs;
                    self.mbc7_clk = new_clk;
                    self.mbc7_eeprom_reg = value;
                } else if self.cartridge_type.is_mbc7() && addr >= 0xA000 && addr < 0xB000 {
                    // MBC7 lower 256 bytes - ignore regular RAM writes
                    // (these are all EEPROM register space)
                } else {
...
impl CartridgeType {
    ...
    pub fn is_mbc7(&self) -> bool {
        matches!(self, CartridgeType::Mbc7)
    }
    ...
    pub fn is_battery_backed(&self) -> bool {
        ...
        match self {
            ...
            | Mbc4RamBattery | Mbc5RamBattery | Mbc5RumbleRamBattery | Mbc7 => true,
            _ => false,
        }
    }
...
impl TryFrom<u8> for CartridgeType {
    ...
    fn try_from(val: u8) -> std::result::Result<Self, Self::Error> {
        match val {
            ...
            0x34 => Ok(CartridgeType::Mbc5RamBattery),
