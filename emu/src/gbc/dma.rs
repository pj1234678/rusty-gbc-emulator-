...
use crate::gbc::ppu::{Ppu, StatMode};
...
impl DmaController {
    pub fn save_to_bytes(&self, buf: &mut Vec<u8>) {
        buf.push(self.oam_dma_counter);
        buf.push(self.hdma_active as u8);
        buf.push(self.hdma_length);
        buf.push(self.hdma_hblank as u8);
        buf.push(self.hdma_chunks_completed);
        buf.push(self.cgb as u8);
    }

    pub fn load_from_bytes(data: &[u8], pos: &mut usize) -> crate::gbc::error::Result<Self> {
        let oam_dma_counter = data[*pos]; *pos += 1;
        let hdma_active = data[*pos] != 0; *pos += 1;
        let hdma_length = data[*pos]; *pos += 1;
        let hdma_hblank = data[*pos] != 0; *pos += 1;
        let hdma_chunks_completed = data[*pos]; *pos += 1;
        let cgb = data[*pos] != 0; *pos += 1;
        Ok(Self { oam_dma_counter, hdma_active, hdma_length, hdma_hblank, hdma_chunks_completed, cgb })
    }
}
