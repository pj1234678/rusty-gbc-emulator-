...
impl Timer {
    ...
    pub fn load_from_bytes(data: &[u8], pos: &mut usize) -> crate::gbc::error::Result<Self> {
        let div = data[*pos]; *pos += 1;
        let div_counter = u16::from_le_bytes([data[*pos], data[*pos + 1]]); *pos += 2;
        let tima = data[*pos]; *pos += 1;
        let tima_counter = u16::from_le_bytes([data[*pos], data[*pos + 1]]); *pos += 2;
        let tma = data[*pos]; *pos += 1;
        let tac = data[*pos]; *pos += 1;
        Ok(Self { div, div_counter, tima, tima_counter, tma, tac })
    }
}
